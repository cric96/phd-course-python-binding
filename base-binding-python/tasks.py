...
def clean(c):
    ...
    for file_pattern in (
        ...
        "./native/*.lib",
        "./native/*.pyd",
        "./native/cffi_example*",  # Is this a dir?
        "./native/cython_wrapper.cpp",
    ):
...
def build_cppmult(c):
    ...
    print_banner("Building C++ Library")
    invoke.run(
        "g++ -O3 -Wall -Werror -shared -std=c++11 -fPIC ./native/cppmult.cpp "
        "-o ./native/libcppmult.so "
    )
    print("* Complete")
...
def build_pybind11(c):
    ...
    compile_python_module("./native/pybind11_wrapper.cpp", "./native/pybind11_example")
    print("* Complete")
...
def build_cython(c):
    ...
    invoke.run("cython --cplus -3 ./native/cython_example.pyx -o ./native/cython_wrapper.cpp")

    # Compile and link the cython wrapper library
    compile_python_module("./native/cython_wrapper.cpp", "./native/cython_example")
    print("* Complete")
